...
- Create two apps on Fly, one for staging and one for production:
  ...
  ```sh
  fly apps create stappa
  fly apps create stappa-staging
  ```
...
- Add a `SESSION_SECRET` to your fly app secrets, to do this you can run the following commands:
  ...
  ```sh
  fly secrets set SESSION_SECRET=$(openssl rand -hex 32) --app stappa
  fly secrets set SESSION_SECRET=$(openssl rand -hex 32) --app stappa-staging
  ```

  > **Note:** When creating the staging secret, you may get a warning from the Fly CLI that looks like this:
  >
  > ```
  > WARN app flag 'stappa-staging' does not match app name in config file 'stappa'
  > ```
  >
...
- Create a database for both your staging and production environments. Run the following:
  ...
  ```sh
  fly postgres create --name stappa-db
  fly postgres attach --app stappa stappa-db

  fly postgres create --name stappa-staging-db
  fly postgres attach --app stappa-staging stappa-staging-db
  ```
